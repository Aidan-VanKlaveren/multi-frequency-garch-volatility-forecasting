## GJRGARCH(1,1) student t fixed df parameter fits over time (200 bins for all freqs) ##
...
out_dir <- "fixed 200 window images"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# Use a constant 200-bin window for every frequency
freq_minutes      <- c(`1M`=1L, `5M`=5L, `15M`=15L, `30M`=30L, `60M`=60L)
window_bins_fixed <- 200L
refit_minutes     <- 60L

pretty_freq <- function(f) recode(f,
                                  `60M`="1-Hour", `30M`="30-Minute", `15M`="15-Minute", `5M`="5-Minute", `1M`="1-Minute", .default=f
)
...
for (f in freqs) {
  ...
  fm          <- freq_minutes[[f]]
  window_bins <- window_bins_fixed                                # CONSTANT 200 bins
  ...
  out_csv_name <- paste0("params_", tolower(pretty_freq(f)), ".csv") %>%
    gsub("-", "", .) %>% gsub(" ", "", .) %>% gsub("Minute", "m", .) %>% gsub("Hour", "h", .)
  out_csv_path <- file.path(out_dir, out_csv_name)
  write_csv(params_f, out_csv_path)
  message("Wrote: ", normalizePath(out_csv_path))
  param_files[[f]] <- out_csv_path
}
...
# Combined CSV
write_csv(params_df, file.path(out_dir, "params_all_frequencies.csv"))
...
# Multi-page PDF (all freqs) in output folder
pdf(file.path(out_dir, "params_trajectories_by_freq.pdf"), width = 8.5, height = 11)
for (ff in levels(p_long$freq_plot)) {
  ...
  print(p_ff)
  ggsave(file.path(out_dir, paste0("params_trajectories_", ff, ".png")), p_ff,
         width = 8, height = 10, dpi = 300)
}
dev.off()

message("Saved to folder: ", normalizePath(out_dir))
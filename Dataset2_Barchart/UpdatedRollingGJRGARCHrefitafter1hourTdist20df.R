## Rolling GJR-GARCH (Student-t, df=20) — refit every hour; 60 one-step forecasts per refit
...
refit_blockwise_stride <- function(train_data, test_data,
                                   window_size = 200L,
                                   block_len   = 60L,
                                   stride      = 60L,
                                   n_starts    = 5L,
                                   df_t        = 20L,   # <-- df set to 20
                                   freq_label  = "") {
  
  cli::cli_inform("===== BLOCKWISE (stride-{stride}) Forecast: {freq_label} (Window = {window_size}, Block = {block_len}, Multi-start = {n_starts}, df={df_t}) =====")
  ...
  # Spec with fixed df (shape = 20)
  gjr_spec_df <- ugarchspec(
    variance.model     = list(model = "gjrGARCH", garchOrder = c(1,1)),
    mean.model         = list(armaOrder = c(0,0), include.mean = FALSE),
    distribution.model = "std",
    fixed.pars         = list(shape = df_t)
  )
  ...
  block_rows <- foreach(bstart = cand, .combine = rbind, .packages = "rugarch") %dorng% {
    tryCatch({
      ...
      # --- Multi-start via n.restarts (random restarts handled internally) ---
      best_fit <- tryCatch(
        ugarchfit(
          gjr_spec_df, data = series_block, out.sample = block_len,
          solver = "gosolnp",
      ...
      # Optional fallback once with hybrid (rarely needed)
      if (is.null(best_fit)) {
        best_fit <- tryCatch(
          ugarchfit(
            gjr_spec_df, data = series_block, out.sample = block_len,
  ...
  # Standardized-t (unit variance) quantile for df=20
  q975 <- rugarch::qdist("std", 0.975, shape = df_t)
  lower <- -q975 * forecast_sd
  upper <-  q975 * forecast_sd
  
  coverage <- mean(r_map >= lower & r_map <= upper, na.rm = TRUE)
...
for (nm in selected_splits) {
  ...
  withCallingHandlers({
    ...
    for (k in seq_along(period_ids)) {
      ...
      res_list[[k]] <- refit_blockwise_stride(
        train_data  = s$train,
        test_data   = s$test,
...
readr::write_csv(all_summaries, "gjr_df20_summaries.csv")
readr::write_csv(all_details,   "gjr_df20_details.csv")

cat("Saved: gjr_df20_summaries.csv and gjr_df20_details.csv\n")


kupiec_test <- function(n, coverage, alpha = 0.05) {
  # n = total forecasts
  # coverage = observed coverage (e.g. 0.9228)
  n_viol <- round(n * (1 - coverage))
  p_hat  <- n_viol / n
  p      <- alpha
  
  # likelihood ratio
  lr_uc <- -2 * ( (n - n_viol) * log(1 - p) + n_viol * log(p)
                  - (n - n_viol) * log(1 - p_hat) - n_viol * log(p_hat) )
  
  p_val <- 1 - pchisq(lr_uc, df = 1)
  list(n = n, violations = n_viol, coverage = coverage,
       lr_stat = lr_uc, p_value = p_val)
}

# Example: your first df20 period
kupiec_test(60208, 0.9228, alpha = 0.05)

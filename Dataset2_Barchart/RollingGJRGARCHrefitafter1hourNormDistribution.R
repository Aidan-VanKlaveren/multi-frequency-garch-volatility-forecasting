## Rolling GJR-GARCH (Gaussian) — refit every hour; 60 one-step forecasts per refit
...
target_prefix <- c("USD")  # extend as needed, e.g. c("USD","EUR","GBP")
...
refit_blockwise_stride_norm <- function(train_data, test_data,
                                        window_size = 200L,
                                        block_len   = 60L,
                                        stride      = 60L,
                                        n_starts    = 5L,
  ...
  cli::cli_inform("===== BLOCKWISE (stride-{stride}) Forecast: {freq_label} (Window = {window_size}, Block = {block_len}, Multi-start = {n_starts}, dist=N(0,1)) =====")
  ...
  block_rows <- foreach(bstart = cand, .combine = rbind, .packages = "rugarch") %dorng% {
    tryCatch({
      ...
      # Multi-start via n.restarts
      best_fit <- tryCatch(
        ugarchfit(
          gjr_spec_norm, data = series_block, out.sample = block_len,
          solver = "gosolnp",
  ...
  # 95% normal bands (standardized normal)
  q975   <- qnorm(0.975)
...
for (nm in selected_splits) {
  ...
  withCallingHandlers({
    ...
    for (k in seq_along(period_ids)) {
      ...
      res_list[[k]] <- refit_blockwise_stride_norm(
        train_data  = s$train,
        test_data   = s$test,
...
# Save
readr::write_csv(all_summaries, "gjr_norm_summaries.csv")
readr::write_csv(all_details,   "gjr_norm_details.csv")

cat("Saved: gjr_norm_summaries.csv and gjr_norm_details.csv\n")
